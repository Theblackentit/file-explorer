...
class LauncherExplorerApp:
    ...
    def load_config(self):
        ...
        for section in merged.get("sections", []):
            ...
            section.setdefault("placement", "home")
            section.setdefault("group", "Recently Added")
            section.setdefault("visual", "poster")
        ...
        if merged["explorer"].get("view") == "Details":
            merged["explorer"]["view"] = "List"
        return merged
    ...
    def build_ui(self):
        ...
        tk.Label(mode_bar, text="View:").pack(side="left")
        self.app_mode_var = tk.StringVar(value="Home")
        ttk.Button(mode_bar, text="Home", command=lambda: self.set_app_mode("Home")).pack(side="left", padx=4)
        ttk.Button(mode_bar, text="Library", command=lambda: self.set_app_mode("Library")).pack(side="left", padx=4)

        body = tk.Frame(self.root)
        body.pack(fill="both", expand=True, padx=12, pady=(0, 10))

        self.nav_sidebar = tk.Frame(body, width=220)
        self.nav_sidebar.pack(side="left", fill="y", padx=(0, 8))

        self.sidebar_title = tk.Label(self.nav_sidebar, text="Shortcuts", font=("Segoe UI", 12, "bold"), anchor="w")
        self.sidebar_title.pack(fill="x", padx=8, pady=(10, 6))
        self.shortcut_list = tk.Listbox(self.nav_sidebar, height=14)
        self.shortcut_list.pack(fill="x", padx=8)
        self.shortcut_list.bind("<Double-1>", self.on_shortcut_double_click)

        tk.Label(self.nav_sidebar, text="Drives", font=("Segoe UI", 12, "bold"), anchor="w").pack(fill="x", padx=8, pady=(12, 6))
        self.drive_list = tk.Listbox(self.nav_sidebar, height=10)
        self.drive_list.pack(fill="both", expand=True, padx=8, pady=(0, 8))
        self.drive_list.bind("<Double-1>", self.on_drive_double_click)

        workspace = tk.Frame(body)
        workspace.pack(side="left", fill="both", expand=True)

        self.main = tk.PanedWindow(workspace, orient="horizontal", sashwidth=8, bd=0)
    ...
    def build_left_side(self):
        ...
        self.left_view_var = tk.StringVar(value="List")
    ...
    def set_app_mode(self, mode):
        ...
        if mode == "Home":
            try:
                self.main.forget(self.left_shell)
            except Exception:
                pass
            if self.right_shell not in self.main.panes():
                self.main.add(self.right_shell)
            self.render_sections()
        else:
            panes = self.main.panes()
            if str(self.left_shell) not in panes:
                self.main.add(self.left_shell)
            if str(self.right_shell) not in panes:
                self.main.add(self.right_shell)
            self.main.sash_place(0, 520, 0)
            self.populate_file_view()

    def refresh_sidebar_lists(self):
        self.shortcut_list.delete(0, "end")
        self.sidebar_shortcut_items = []
        for section in self.config_data.get("sections", []):
            if section.get("placement", "home") == "shortcut":
                self.sidebar_shortcut_items.append(section)
                self.shortcut_list.insert("end", section.get("name", "Shortcut"))
        ...
        if os.name == "nt":
            ...
            drives = ["/"]
        for d in drives:
            self.drive_items.append(d)
            self.drive_list.insert("end", d)

    def on_shortcut_double_click(self, _event):
        if not self.shortcut_list.curselection():
            return
        idx = self.shortcut_list.curselection()[0]
        if 0 <= idx < len(getattr(self, "sidebar_shortcut_items", [])):
            self.open_section(self.sidebar_shortcut_items[idx])

    def on_drive_double_click(self, _event):
        if not self.drive_list.curselection():
            return
        idx = self.drive_list.curselection()[0]
        if 0 <= idx < len(getattr(self, "drive_items", [])):
            self.set_app_mode("Library")
            self.set_current_dir(self.drive_items[idx])
    ...
    def populate_file_view(self):
        ...
        if mode == "List":
            ...
            self.file_view.column("#0", width=420)
        ...
        row_h = max(22, int(self.left_size_var.get()) + (12 if mode == "Icons" else (8 if mode == "Tiles" else 2)))
        ...
        for name in names:
            ...
            if mode == "Tiles":
                tile_text = f"{name}\n{typ} • {size or '-'} • {modified}"
                self.file_view.insert("", "end", text=tile_text, image=icon, values=(size, typ, modified, full))
            elif mode != "Icons":
    ...
    def section_editor_window(self, index=None):
        ...
        tk.Label(prow, text="Placement", width=14, anchor="w").pack(side="left")
        placement_var = tk.StringVar(value=source.get("placement", "home"))
        ttk.Combobox(prow, textvariable=placement_var, values=["home", "shortcut"], state="readonly").pack(side="left", fill="x", expand=True)

        grow = tk.Frame(left)
        grow.pack(fill="x", pady=5)
        tk.Label(grow, text="Section row", width=14, anchor="w").pack(side="left")
        group_var = tk.StringVar(value=source.get("group", "Recently Added"))
        ttk.Entry(grow, textvariable=group_var).pack(side="left", fill="x", expand=True)

        vrow = tk.Frame(left)
        vrow.pack(fill="x", pady=5)
        tk.Label(vrow, text="Card style", width=14, anchor="w").pack(side="left")
        visual_var = tk.StringVar(value=source.get("visual", "poster"))
        ttk.Combobox(vrow, textvariable=visual_var, values=["poster", "banner"], state="readonly").pack(side="left", fill="x", expand=True)
        ...
        def save():
            payload = {
                ...
                "placement": placement_var.get().strip() or "home",
                "group": group_var.get().strip() or "Recently Added",
                "visual": visual_var.get().strip() or "poster",
                "image": image_var.get().strip(),
                "shape": shape_var.get().strip(),
                "image_mode": mode_var.get().strip(),
    ...
    def render_sections(self):
        ...
        for idx, section in sorted_sections:
            if section.get("placement", "home") != "home":
                continue
            group = section.get("group", "Recently Added")
            groups.setdefault(group, []).append((idx, section))

        row_index = 0
        for group_name, items in groups.items():
            header = tk.Label(self.cards_container, text=group_name, font=("Segoe UI", 18, "bold"), anchor="w")
            header.grid(row=row_index, column=0, sticky="w", padx=8, pady=(6, 10))
            row_index += 1

            row_frame = tk.Frame(self.cards_container)
            row_frame.grid(row=row_index, column=0, sticky="w", padx=4, pady=(0, 14))
            row_index += 1

            col = 0
            for idx, section in items:
                visual = section.get("visual", "poster")
                if visual == "banner":
                    card_w = int(430 * zoom)
                    card_h = int(250 * zoom)
                    img_w = int(392 * zoom)
                ...
                card = tk.Frame(row_frame, width=card_w, height=card_h, bd=0, relief="flat")
                card.grid(row=0, column=col, padx=8, pady=4, sticky="n")
                card.grid_propagate(False)
                col += 1
